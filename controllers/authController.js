...
//       return res.status(400).json({
//         success: false,
//         msg: "Mobile number, OTP, and role are required",
//       });
//     }
...
//       });
//     }
//     // Check if user exists or create/update user
 ...
 * @param {Object} req - Express request object with mobileNumber in body
...
//     // Return success with token and user info (excluding sensitive data)
//     // return res.status(200).json({
...
//     //     // Add other non-sensitive user fields you want to return
//     //   }
//     // });
...
console.log(username,password,"==========");
    ...
    console.log(user,"uuuuuuuuuuuuppppppppvvvvvvv");
    if (!user) {
      return res.status(401).json({
        success: false,
    ...
    // Verify password
    const isPasswordValid = await bcrypt.compare(password, user.password);
    console.log(isPasswordValid,"ppppppppvvvvvvv");
    ...
    if (user.roles.length === 1|| !isPartnerLogin) {
      ...
      roleToReturn = isPartnerLogin&&user.roles.find(role => role.toLowerCase() !== 'user') || user.roles[0];
    }
...
/**
 * Verifies OTP and handles user authentication
 * @param {Object} req - Express request object with mobileNumber and otpCode in body
 * @param {Object} res - Express response object
 * @returns {Object} Response with token and user data if successful
 */
const verifyOTPHandler = async (req, res) => {
  try {
    const { mobileNumber, otpCode } = req.body;
    ...
    if (freshUser.roles.length === 1) {
      ...
      // Find the first non-"user" role if multiple roles exist
      roleToReturn = freshUser.roles.find(role => role.toLowerCase() !== 'user') || freshUser.roles[0];
    }
...
export { sendOTPHandler, verifyOTPHandler ,login, updateCredentials};